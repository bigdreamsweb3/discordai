...
async function waitForChatMessages(page) {
  const MAX_TOTAL_WAIT = 180000; // 3 minutes max
  ...
  while (true) {
    try {
      ...
      const isVisible = await page.evaluate(() => {
        const el = document.querySelector('ul[data-list-id="chat-messages');
        if (!el) return false;
      ...
      if (isVisible) {
        ...
        await plugins.wait.forLoad({ timeout: 30000 });
        return; // SUCCESS
      }
    } catch (_) {
    ...
    if (Date.now() - start > MAX_TOTAL_WAIT) {
      throw new Error("Timed out waiting for chat messages to become visible");
    }

    await plugins.wait.forLoad({ timeout: CHECK_INTERVAL });
  }
}
...
async function extractUser(page, username, channelId, messageId, options = {}) {
  ...
  try {
    ...
    if (!currentUrl.includes(channelId)) {
      log("Navigating to target channel...");
      await navigateToChannel(page, channelId);
    }

    // 🔹 Build safe screenshot path
    const safeName = cleanName.replace(/[^a-zA-Z0-9]/g, "_");
    const waitShotPath = path.join(
      baseDir,
      `wait-shot-${safeName}-${Date.now()}.png`
    );

    // 🔹 Take screenshot before waiting (debug aid)
    await page.screenshot({ path: waitShotPath, fullPage: true });

    // 🔹 Ensure messages are fully loaded
